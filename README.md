...
1. **No math.** You get "🚨 $WOJAK buying pressure!" with no idea why. PulseLock shows
   the score *components* on the alert, in the dashboard, and in the terminal, so you can
...
result, summed and clamped to 100. Straight from `src/score.ts`, nothing else:
...
| Var | Default | Purpose |
|---|---|---|
| `SOLAMI_API_KEY` | (none) | **Required.** Blur WS auth + gRPC token fallback |
...
| `DISCORD_WEBHOOK_URL` | (none) | alert sink |
| `TELEGRAM_BOT_TOKEN` | (none) | alert sink + self-serve subscriber poll |
| `TELEGRAM_CHAT_ID` | (none) | optional bootstrap subscriber |
| `TELEGRAM_BOT_USERNAME` | (none) | shown on the dashboard CTA |
